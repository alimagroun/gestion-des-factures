...
public class PdfController {
  ...
  /*  @GetMapping(value = "/generate-invoice", produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<byte[]> generateInvoicePdf() {
        String invoiceContent = invoiceService.generateInvoicePdfContent();
        byte[] pdfBytes = PdfUtils.generatePdfBytes(invoiceContent);
    ...
    public ResponseEntity<byte[]> generateInvoicePdf(@PathVariable Long invoiceId) {
        ...
        String filename = "invoice_" + invoiceId + ".pdf"; // Customize filename based on invoiceId
